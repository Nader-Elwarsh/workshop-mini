...
function deviceProfile(){let el=document.getElementById("deviceProfile");if(!el)return;let d=arr(K.d).find(x=>x.id===new URLSearchParams(location.search).get("id"));if(!d){el.innerHTML="<div class='item'>الجهاز غير موجود.</div>";return}let rs=arr(K.r).filter(r=>r.deviceId===d.id);let acc=deviceAccountSummary(d.id);el.innerHTML=`<div class="profile"><div class="page-head"><h1 class="profile-title">🔧 ${esc(d.type)} — ${esc(d.brand)}</h1><div class="compact-actions"><button class="secondary" onclick="editDevice('${d.id}')">✏️ تعديل</button><a class="primary" href="requests.html?customer=${d.customerId}&device=${d.id}">➕ أمر شغل</a></div></div><div class="profile-grid"><div class="kv"><b>👤 العميل</b><a href="customer.html?id=${d.customerId}">${esc(customerName(d.customerId))}</a></div><div class="kv"><b>📍 العنوان</b>${esc(addressText((arr(K.c).find(c=>c.id===d.customerId)||{}).mainAddress||{}))}</div><div class="kv"><b>النوع / التصنيف</b>${esc(d.type)} / ${esc(d.category)}</div><div class="kv"><b>الموديل</b>${esc(d.model)||"—"}</div></div>${d.photo?`<img class="photo" src="${d.photo}">`:""}${accountSummaryHtml(acc,"💰 حساب الجهاز")}<h2>🛠️ سجل أوامر الشغل</h2>${rs.length?rs.map(r=>`<div class="item"><a href="request.html?id=${r.id}"><b>${esc(r.no)}</b></a> <span class="badge">${esc(r.status)}</span>${r.workshopStatus&&r.workshopStatus!=="غير مطلوب"?`<span class="badge workshop-badge">🏭 ${esc(r.workshopStatus)}</span>`:""}<div>${esc(r.fault)}</div></div>`).join(""):"<div class='item'>لا يوجد سجل.</div>"}${printShareBar("Device",d.id)}</div>`}
...
function addExternalPartToOrder(){let nameEl=document.getElementById("rExtPartName"),priceEl=document.getElementById("rExtPartPrice"),qtyEl=document.getElementById("rExtPartQty");let name=(nameEl.value||"").trim(),price=+priceEl.value||0,qty=+(qtyEl.value||1);if(!name)return alert("اكتب اسم القطعة.");if(price<=0)return alert("اكتب سعر القطعة.");if(!Number.isFinite(qty)||qty<1)return alert("اكتب كمية صحيحة.");currentParts.push({external:true,name,qty,sell:price,cost:price});renderOrderParts();calc();nameEl.value="";priceEl.value="";qtyEl.value=1}
function renderOrderParts(){let el=document.getElementById("orderParts");el.innerHTML=currentParts.map((x,i)=>{let label=x.external?`🏷️ ${esc(x.name)} <small>(خارج المخزن)</small>`:esc((arr(K.p).find(z=>z.id===x.partId)||{}).name);return `<div class="part-row"><span>${label}</span><input type="number" min="1" value="${x.qty}" onchange="currentParts[${i}].qty=+this.value;calc();renderOrderParts()"><span>${(x.qty*x.sell).toFixed(2)} ج</span><button type="button" class="secondary" onclick="currentParts.splice(${i},1);renderOrderParts();calc()">🗑️</button></div>`}).join("")}
...
function confirmAddExternalPartToRequest(requestId){let rs=arr(K.r),r=rs.find(x=>x.id===requestId);if(!r)return alert("أمر الشغل غير موجود.");if(r.closed||r.paid)return alert("الأمر مغلق أو مدفوع بالكامل ولا يمكن إضافة قطع غيار.");let nameEl=document.getElementById("rpExtName"),priceEl=document.getElementById("rpExtPrice"),qtyEl=document.getElementById("rpExtQty");let name=(nameEl.value||"").trim(),price=+priceEl.value||0,qty=+(qtyEl.value||1);if(!name)return alert("اكتب اسم القطعة.");if(price<=0)return alert("اكتب سعر القطعة.");if(!Number.isFinite(qty)||qty<1)return alert("اكتب كمية صحيحة.");let updatedParts=(r.parts||[]).map(x=>({...x}));updatedParts.push({external:true,name,qty,sell:price,cost:price});let partsTotal=updatedParts.reduce((a,x)=>a+(+x.qty||0)*(+x.sell||0),0),partsCost=updatedParts.reduce((a,x)=>a+(+x.qty||0)*(+x.cost||0),0),total=(+r.labor||0)+partsTotal;let updated={...r,parts:updatedParts,partsTotal,partsCost,total,remain:Math.max(0,total-(+r.deposit||0))};put(K.r,rs.map(x=>x.id===r.id?updated:x));requestProfile()}

/* ---------------------------------------------------------------------
   🖨️ طباعة / حفظ PDF و📤 مشاركة — من غير سيرفر، نستخدم طباعة المتصفح
   (اللي بتدّي خيار "حفظ كـ PDF" أصلًا) وواجهة المشاركة الأصلية للجهاز.
--------------------------------------------------------------------- */
function printPage(){window.print()}
async function sharePageText(title,text){
  if(navigator.share){try{await navigator.share({title,text});return}catch(e){return}}
  if(navigator.clipboard){try{await navigator.clipboard.writeText(text);alert("مفيش مشاركة مباشرة على المتصفح ده، فتم نسخ التفاصيل — تقدر تلصقها في أي محادثة.");return}catch(e){}}
  alert(text);
}
function printShareBar(kind,id){return `<div class="print-share-bar"><button type="button" class="secondary" onclick="printPage()">🖨️ طباعة / حفظ PDF</button><button type="button" class="secondary" onclick="share${kind}('${id}')">📤 مشاركة</button></div>`}
async function shareRequest(rid){let r=arr(K.r).find(x=>x.id===rid);if(!r)return;let text=`🛠️ أمر شغل ${r.no}\nالعميل: ${customerName(r.customerId)}\nالجهاز: ${deviceName(r.deviceId)}\nالعطل: ${r.fault||"—"}\nالمصنعية: ${(+r.labor||0).toFixed(2)} ج\nقطع الغيار: ${(+r.partsTotal||0).toFixed(2)} ج\nالإجمالي: ${(+r.total||0).toFixed(2)} ج\nالعربون: ${(+r.deposit||0).toFixed(2)} ج\nالحالة: ${r.status}${r.closed?" (مغلق)":""}`;await sharePageText(`أمر شغل ${r.no}`,text)}
async function shareCustomer(cid){let c=arr(K.c).find(x=>x.id===cid);if(!c)return;let acc=customerAccountSummary(cid);let text=`👤 ${c.name}\n📞 ${c.phone}\n📍 ${addressText(c.mainAddress||{})}\n\n💰 حساب العميل:\nإجمالي الأوامر: ${acc.total.toFixed(2)} ج\nالمحصّل: ${acc.collected.toFixed(2)} ج\nالمتبقي: ${acc.remain.toFixed(2)} ج`;await sharePageText(`ملف العميل - ${c.name}`,text)}
async function shareDevice(did){let d=arr(K.d).find(x=>x.id===did);if(!d)return;let acc=deviceAccountSummary(did);let text=`🔧 ${d.type} — ${d.brand}\n👤 العميل: ${customerName(d.customerId)}\nالموديل: ${d.model||"—"}\n\n💰 حساب الجهاز:\nإجمالي الأوامر: ${acc.total.toFixed(2)} ج\nالمحصّل: ${acc.collected.toFixed(2)} ج\nالمتبقي: ${acc.remain.toFixed(2)} ج`;await sharePageText(`ملف الجهاز - ${d.type} ${d.brand}`,text)}
function customerAccountSummary(cid){let rs=arr(K.r).filter(r=>r.customerId===cid);let total=rs.reduce((a,r)=>a+(+r.total||0),0);let remain=rs.reduce((a,r)=>a+(r.closed?0:Math.max(0,(+r.total||0)-(+r.deposit||0))),0);return{total,remain,collected:total-remain,count:rs.length}}
function deviceAccountSummary(did){let rs=arr(K.r).filter(r=>r.deviceId===did);let total=rs.reduce((a,r)=>a+(+r.total||0),0);let remain=rs.reduce((a,r)=>a+(r.closed?0:Math.max(0,(+r.total||0)-(+r.deposit||0))),0);return{total,remain,collected:total-remain,count:rs.length}}
function accountSummaryHtml(acc,title){return `<div class="account-summary"><h3>${title||"💰 الحساب"}</h3><table class="month-table compact-money"><tr><td>🛠️ عدد الأوامر</td><td>${acc.count}</td></tr><tr><td>💰 إجمالي قيمة الأوامر</td><td>${acc.total.toFixed(2)} ج</td></tr><tr><td>✅ المحصّل</td><td>${acc.collected.toFixed(2)} ج</td></tr><tr class="total-row"><td>🧾 المتبقي</td><td>${acc.remain.toFixed(2)} ج</td></tr></table></div>`}
...
(function setupPWA(){
  ...
  if('serviceWorker' in navigator){
    window.addEventListener('load',()=>navigator.serviceWorker.register('./service-worker.js?v=11.9.0', {updateViaCache: 'none'}).catch(err=>console.warn('PWA service worker:',err)));
  }
